...
vector<double> MDP::computeValue(const vector<int> &policy){
  ...
  do{
	...
	continue;
      ...
      double self = value[i];
      double northValue = selfStateProb*(cell[i].reward + self) + (1-selfStateProb)*((1.0 - actionNoiseProb) * n + actionNoiseProb * e);
      double eastValue = selfStateProb*(cell[i].reward + self) + (1-selfStateProb)*((1.0 - actionNoiseProb) * e + actionNoiseProb * n);
