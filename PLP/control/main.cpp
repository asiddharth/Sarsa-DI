...
double evaluate(MDP *m, Agent *agent, const int &numEpisodes, int step){
  ...
  for(int i = 0; i < numEpisodes; i++){
    ...
    do{
      ...
      if (steps % step == 0) {
          action = agent->takeBestAction(features);
      }
...
int main(int argc, char *argv[]){
    ...
    agent = new SarsaLambdaAgent(testMDP->getNumFeatures(), 2, totalEpisodes, initWeight, lambda, alphaInit, epsInit, randomSeed);
      ...
      if(pos != string::npos){
        ...
        if(pos != string::npos){
          ...
          pos = method.find("_", pos + 1);
          if(pos != string::npos){
            persStep = atof((method.substr(pos + 1, method.length() - pos)).c_str());
          }
        }
        else{
          options(); return 1;
      ...
      agent = new TDLambdaNStepAgent(testMDP->getNumFeatures(), 2, totalEpisodes, initWeight, lambda, alphaInit, epsInit, randomSeed, (int)persStep);
  }
        ...
        if(pos != string::npos){
          ...
          if(pos != string::npos){
            ...
            pos = method.find("_", pos + 1);
            if(pos != string::npos){
              nStep = atof((method.substr(pos + 1, method.length() - pos)).c_str());
            }
          }
          else{
            options(); return 1;
        ...
        options(); return 1;
          ...
          agent = new TDNStep(testMDP->getNumFeatures(), 2, totalEpisodes, initWeight, lambda, alphaInit, epsInit, randomSeed, (int)nStep);
  ...
  for(unsigned int e = 0; e <= totalEpisodes; e++){
    ...
    if(points.find(e) != points.end()){
      ...
      double val = evaluate(testMDP, agent, numEvalEpisodes,  (int)persStep);
