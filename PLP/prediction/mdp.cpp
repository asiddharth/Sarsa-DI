...
int MDP::getActionOpt() {
	return cell[currentState].maximalAction;
}
...
vector<double> MDP::computeValue(const vector<int> &policy){
  ...
  do{
	...
	continue;
      ...
      double self = value[i];
      double northValue = selfStateProb*(self + cell[i].reward) + (1-selfStateProb)*((1.0 - actionNoiseProb) * n + actionNoiseProb * e);
      double eastValue = selfStateProb*(self + cell[i].reward) + (1-selfStateProb)*((1.0 - actionNoiseProb) * e + actionNoiseProb * n);
...
vector<int> MDP::obtainMinPolicy(const vector<double> &value){
